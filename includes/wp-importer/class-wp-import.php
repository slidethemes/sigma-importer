...
class WP_Import extends WP_Importer{
	...
	/**
   * Method used to import theme_options.json files.
   *
   * @since    1.0.0
   */
  function import_theme_options( $file, $opt_name ) {
		...
		if ( !$fs->exists( $file ) ) {
			return new \WP_Error(
				'redux_import_file_not_found',
				__( 'Redux import file could not be found.', 'ss-importer' )
			);
		}

    $data = $fs->get_contents( $file );
    ...
    if ( is_array( $data ) && !empty( $data ) ) {
      ...
      return true;
		...
		return new \WP_Error(
			'corrupted_redux_import_data',
			__( 'Redux import data could not be read. Please try a different file.', 'ss-importer' )
		);
	...
	function import( $file, $options ) {
		...
		$import_start = $this->import_start( $file );

		if ( is_wp_error( $import_start ) ) {
			$this->logger->error( __( 'Content import error: ', 'ss-importer' ) . $import_start->get_error_message() );
			return false;
		}
		...
		/* Loop through the option methods that are supported, the below methods are supported
		*
		*	process_categories
		*	process_tags
		*	process_terms
	...
	function import_start( $file ) {
		...
		// File exists?
		if ( !$fs->exists( $file ) ) {
			return new WP_Error( 'broke', __( 'The content file does not exist', 'ss-importer' ) );
		}

		$import_data = $this->parse( $file );

		if ( is_wp_error( $import_data ) ) {
			$this->logger->error( __( 'Content import parse error: ', 'ss-importer' ) . $import_data->get_error_message() );
			die();
		}
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Failed to import author %s. Their posts will be attributed to the current user.', 'ss-importer' ),
		...
		foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
			...
			if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Failed to create new user for %1$s. Their posts will be attributed to the current user: %2$s', 'ss-importer' ),
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) && $id > 0 ) {
				...
				$this->logger->warning( sprintf(
					__( 'Failed to insert category %1$s. The following error occured: %2$s', 'ss-importer' ),
	...
	function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				$this->logger->warning( sprintf(
					__( 'Failed to insert tag %1$s. The following error occured: %2$s', 'ss-importer' ),
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				$this->logger->warning( sprintf(
					__( 'Failed to insert term %1$s. The following error occured: %2$s', 'ss-importer' ),
			...
			*/
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				...
				if ( is_wp_error( $post_id ) ) {
					$this->logger->warning( sprintf(
						__( 'Failed to process %s, the following error occured %s', 'ss-importer' ),
						$post['post_title'],
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							$this->logger->warning( sprintf(
							  __( 'Failed to import %s %s', 'ss-importer' ),
							  $taxonomy,
							  $term['name']
							) );
							$this->logger->debug( $t->get_error_message() );
	...
	function process_attachment( $post, $url ) {
		if ( ! $this->fetch_attachments )
			return new WP_Error( 'attachment_processing_error',
				__( 'Fetching attachments is not enabled', 'ss-importer' ) );
		...
		if ( $info = wp_check_filetype( $upload['file'] ) )
			...
			return new WP_Error( 'attachment_processing_error', __('Invalid file type', 'ss-importer') );
	...
	function fetch_remote_file( $url, $post ) {
		...
		if ( ! $tmp_file_name ) {
			return new WP_Error( 'import_no_file', __( 'Could not create temporary file.', 'ss-importer' ) );
		}
		...
		if ( is_wp_error( $remote_response ) ) {
			...
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Request failed due to an error: %1$s (%2$s)', 'ss-importer' ),
		...
		if ( 200 !== $remote_response_code ) {
			...
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Remote server returned the following unexpected result: %1$s (%2$s)', 'ss-importer' ),
		...
		if ( ! $headers ) {
			@unlink( $tmp_file_name );
			return new WP_Error( 'import_file_error', __('Remote server did not respond', 'ss-importer') );
		}
		...
		if ( 0 === $filesize ) {
			@unlink( $tmp_file_name );
			return new WP_Error( 'import_file_error', __('Zero size file downloaded', 'ss-importer') );
		}

		if ( ! isset( $headers['content-encoding'] ) && isset( $headers['content-length'] ) && $filesize !== (int) $headers['content-length'] ) {
			@unlink( $tmp_file_name );
			return new WP_Error( 'import_file_error', __('Downloaded file has incorrect size', 'ss-importer' ) );
		}

		$max_size = (int) $this->max_attachment_size();
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			@unlink( $tmp_file_name );
			return new WP_Error( 'import_file_error', sprintf(__('Remote file is too large, limit is %s', 'ss-importer' ), size_format($max_size) ) );
		}
		...
		if ( ( ! $type || ! $ext ) && ! current_user_can( 'unfiltered_upload' ) ) {
			return new WP_Error( 'import_file_error', __( 'Sorry, this file type is not permitted for security reasons.', 'ss-importer' ) );
		}
		...
		if ( ! $move_new_file ) {
			@unlink( $tmp_file_name );
			return new WP_Error( 'import_file_error', __( 'The uploaded file could not be moved', 'ss-importer' ) );
		}
