<?php
/**
 * The Autoloader class for SS_Importer.
 */
class SS_Autoloader {
  ...
  public function __construct( $args = array() ) {
      ...
      if ( empty( self::$transient_name ) ) {
          self::$transient_name = 'ss_importer_autoloader_paths_' . md5( __FILE__ );
      }
  ...
  protected function get_path( $class_name ) {
      ...
      $paths = array();
      if ( 0 === stripos( $class_name, 'SS_' ) ) {
          $path     = wp_normalize_path( SS_IMPORTER_PATH . 'includes/' );

          $class_name = str_replace( array( 'SS_' ), '', $class_name );

          $filename = 'SS_' . $class_name . '.php';
